import "./vendorproductsall.component.css"
import { useEffect, useRef, useState } from "react"
import { useNavigate } from "react-router-dom"
import { deleteServiceData, getServiceData, getVendorServiceData, updateApproveService } from "../../../services/vendorservices.service"
import { handleApprovePayment } from "../vendorpayments/vendorpayments.component"
...
export function VendorProductAllComponent(props) {
    ...
    function handleGetProjectData() {
        ...
        //    axiosPrivate.get('http://localhost:4006/getvendorservice')
            ...
            .catch((error)=>{
                ...
                alert(error.response.data + "from BE") // token expire
    ...
    function editItem(item) {
        props.handleProductEditData(item)
    }
    ...
    function handleGiveApprovalPayment(id){
        // handlePayment(id,handleGiveApproval)
        handleApprovePayment(id,handleGiveApproval)
    ...
    let pendingShow = projectGridBodyData.filter((item) => item.isapproved === 'false' || item.isapproved === null);

    // let pendingShow = projectGridBodyData.filter((item) => !item.isapproved)
    console.log(pendingShow)
    return (
        <>
            <div className="container-fluid">
                <div className="project_all_header row pb-3">
                    <div className=" col-auto col-sm-2">
                        <button className="btn-create" onClick={() => props.onMenuItemClick('vendorserviceadd')}>Create Product</button>
                    </div>
                ...
                <div >
                    {
                        pendingShow.length !== 0 && <div className="pending-styles p-3 mb-3" >
                            <table class="table table-success table-bordered table-hover ">
                                <h5 className="text-danger">You have pending products...</h5>
                                <tbody>
                                    {
                                        projectGridBodyData.filter((item) => item.isapproved === 'false' || item.isapproved === null).map(
                                            (item, index) => {
                                                return <>
                                                    <tr key={item.id} className="">
                                                        {/* <td className="text-center ckeckbox_table"> <input type="checkbox" value={item.id} checked={isChecked[index]} onChange={(e) => handleChecked(e, index)} style={{}} className="checkbox_styles pt-3 ps-3"></input> </td> */}
                                                        <td className="text-whit fw-bold">
                                                            {item.name} <br></br>
                                                            {item.description}<br></br>
                                                            {item.location}

                                                        </td>
                                                        {/* <td className="text-whit">
                        {item.location}
                    </td>
                    <td className="text-whit" >
                        {item.description}
                                                        ...
                                                        <td className="text-whit fw-bold">
                                                            <span>Your product placement is pending, <br></br>To place your product please complete payment</span>
                                                        </td>
                                                        <td >
                                                            <div className="d-flex justify-content-center project_btn p-0 m-0">
                                                                {/* <button className="btn_edit p-1 ps-2 pe-2" onClick={() => (editItem(item), props.onMenuItemClick('productedit'))} ><i className="fa-solid fa-pen"></i></button>&nbsp; */}
                                                                ...
                                                                <button className="btn btn-warning" onClick={() => { handleGiveApprovalPayment(item._id) }}>Make Payment</button>
                    ...
                    <div className="p-3 pt-2">
                        <div className="project_all_grid ">
                            ...
                            <table className='table table-bordered table-hover' >
                                ...
                                <tbody>
                                    {
                                        projectGridBodyData.filter(item => item.isapproved === 'true').map(
                                            (item, index) => {
                                                return <>
                                                    <tr key={item._id} className="">
                                                        ...
                                                        <td>
                                                            <div className="d-flex justify-content-center project_btn p-0 m-0">
                                                                ...
                                                                {
                                                                   item.isapproved === null && <button className="btn btn-warning" onClick={() => { handleGiveApproval(item.id) }}>Give Approval</button>
                                                                }
                                                                 {
                                                                    item.isapproved === 'false' && <button className="btn btn-warning" onClick={() => { handleGiveApproval(item.id) }}>Give Approval</button>
                                                                }&nbsp;&nbsp;
